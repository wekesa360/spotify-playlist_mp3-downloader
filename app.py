    ...
    if request.method == 'POST':
        for key in playlist_dict.items():
            if request.form.get('view_tracks_button') == key[0]:
                ...
                return render_template('tracks.html', tracks=tracks, user_name=user_name, user_images=user_images)
            ...
            if request.form.get('download_button') == key[0]:
                ...
                return render_template('download_playlist.html', file_name=playlist_name, download_zip=download_zip, playlist_cover_img=playlist_cover_img)

    with app.app_context():
        rendered = render_template('playlists.html', playlists=playlists_names, user_name=user_name, user_images=user_images)
    return rendered
